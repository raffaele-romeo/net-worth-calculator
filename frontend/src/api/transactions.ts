...
export function getTransactionsByAssetId(assetId: number, year?: number, currency?: string): Promise<AggregatedTransactions[]> {
    const params = new URLSearchParams();
    if (year) params.set("year", year.toString());
    if (currency) params.set("currency", currency);
    const query = params.toString();
    return get<AggregatedTransactions[]>(`/assets/${assetId}/transactions${query ? `?${query}` : ""}`);
}

export function getTransactionsByAssetType(assetType: string, year?: number, currency?: string): Promise<AggregatedTransactions[]> {
    const params = new URLSearchParams();
    if (year) params.set("year", year.toString());
    if (currency) params.set("currency", currency);
    const query = params.toString();
    return get<AggregatedTransactions[]>(`/assets/${assetType}/transactions${query ? `?${query}` : ""}`);
}
