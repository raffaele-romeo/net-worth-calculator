...
object JWTAuthMiddleware:
  ...
  def apply[F[_], A: Show](
    ...
    val authUser: Kleisli[F, Request[F], Either[String, A]] = Kleisli {
      request =>
        AuthHeaders
          .getBearerToken(request)
          .fold("Bearer token not found".asLeft[A].pure[F]) { token =>
            S.delay(SignedJWT.parse(token.toString))
              ...
              .map {
                _.fold("Bearer token not found or invalid".asLeft[A])(
                  _.asRight[String]
                )
              }
          }
